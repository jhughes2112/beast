...
public class TestCaptureTransport : ITransportServer
{
	...
	// Stats also lands in a typed list: the frame's numbers carry an invariant (cached + input +
	// output is the whole context) that tests assert directly, and re-parsing them out of the
	// flattened text would only be a way to get that wrong.
	private readonly List<(int Input, int Output, int MaxContext, int Context, int Cached)> _stats = new List<(int Input, int Output, int MaxContext, int Context, int Cached)>();

	public IReadOnlyList<(int Input, int Output, int MaxContext, int Context, int Cached)> StatFrames => _stats;
	...
	{
		_stats.Add((promptTokens, completionTokens, maxContext, contextTokens, cachedTokens));
