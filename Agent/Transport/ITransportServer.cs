...
public enum FrameType : byte
{
	...
	SessionActivate = 19, // agent asks the client to show this session; the client keeps every session it already knows
...
public interface ITransportServer
{
	...
	// Asks the client to make this session the one on screen. Sent when the agent stands up a session
	// the user should be looking at: a compaction successor, or a fresh root after the active one was
	// deleted. The client keeps everything else it knows — the agent never tells it to forget.
	//
	// This used to be SessionReset, which cleared the client's entire session list. That made sense
	// when a session could be rewritten in place, but compaction appends a successor now and nothing
	// is ever replaced, so there is nothing to invalidate. The client owns its own list: it removes
	// sessions when the USER deletes them, which it already does locally before the agent is told.
	void SessionActivate(string sessionId);
