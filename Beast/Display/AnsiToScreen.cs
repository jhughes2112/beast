...
public static class AnsiToScreen
{
	...
	public static (int EndX, Rgb? FinalFg, Rgb? FinalBg) WriteLine(Screen target, int x0, int y, string ansiText, Rgb? baseFg, Rgb? baseBg)
	{
		...
		while (i < ansiText.Length)
		{
			...
			{
				int j = i + 2;
				while (j < ansiText.Length && !(ansiText[j] >= '@' && ansiText[j] <= '~'))
					j++;
		...
		Rgb? baseFg, Rgb? baseBg)
	{
		...
		while (k < p.Count)
		{
			...
			switch (code)
			{
				...
				default:
					if (code >= 30 && code <= 37)
					{ fg = StandardAnsi(code - 30, bright: false); k++; }
					else if (code >= 40 && code <= 47)
					{ bg = StandardAnsi(code - 40, bright: false); k++; }
					else if (code >= 90 && code <= 97)
					{ fg = StandardAnsi(code - 90, bright: true); k++; }
					else if (code >= 100 && code <= 107)
					{ bg = StandardAnsi(code - 100, bright: true); k++; }
					else
					{ k++; }
					break;
			}
