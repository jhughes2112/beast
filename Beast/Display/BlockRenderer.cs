...
internal static class BlockRenderer
{
	...
	internal static BlockLayer Build(DisplayMessage msg, int w, bool plainText)
	{
		...
		if (contentW > MaxBlockWidth)
			contentW = MaxBlockWidth;
	...
	{
		if (isError)
			return (DisplayScreen.Palette.ToolCallErrFg, DisplayScreen.Palette.ToolCallErrBg);
		switch (type)
		{
			case FrameType.Output:
				return (DisplayScreen.Palette.Silver, null);
			case FrameType.User:
			...
			case FrameType.Tool:
				return (DisplayScreen.Palette.Blue, null);
			case FrameType.ToolCall:
				return (DisplayScreen.Palette.ToolCallFg, DisplayScreen.Palette.ToolCallBg);
			case FrameType.ToolResponse:
				return (DisplayScreen.Palette.ToolRespFg, DisplayScreen.Palette.ToolRespBg);
			case FrameType.System:
				return (DisplayScreen.Palette.Orange, null);
			case FrameType.Debug:
				return (DisplayScreen.Palette.MedGrey, null);
			default:
				return (DisplayScreen.Palette.Silver, null);
		}
	}
	...
	private static List<string> RenderToolCallRows(DisplayMessage msg, int w, out int responseStart, out int errorStart)
	{
		...
		if (name != "edit_file" && !string.IsNullOrWhiteSpace(argsJson))
		{
			try
			{
				...
				{
					if (summaryProps.Contains(prop.Name))
						continue;
	...
	{
		if (toolName == "return_to_caller" && propName.Equals("output", StringComparison.OrdinalIgnoreCase))
			return true;
		if (toolName == "finish_review" && propName.Equals("comments", StringComparison.OrdinalIgnoreCase))
			return true;
		...
		if (toolName == "task_complete" && propName.Equals("results_of_review_work", StringComparison.OrdinalIgnoreCase))
			return true;
		if (toolName == "state_transition" && propName.Equals("context", StringComparison.OrdinalIgnoreCase))
			return true;
		...
		if ((toolName == "assign_work" || toolName == "review_work") && propName.Equals("prompt", StringComparison.OrdinalIgnoreCase))
			return true;
